...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  //status 'Typing' meaning user is typing, 'Idle' meaning user is not typing
  async updateStatus(socket : Socket, receiverId : string, status : "Typing" | "Idle") {
    const user = this.activeUsers.get(socket.id);
  ...
  async handleMessage(socket : Socket, receiverId : string, data : 
    {
      type : "send",
      data : Chat
    } | 
